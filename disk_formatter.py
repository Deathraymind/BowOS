...
class DiskFormatterApp(App):
    ...
    async def format_disk(self, disk: str):
        ...
        try:
            ...
            await self.run_command(f"sed -i 's/export BOWOS_USER=bowyn/export BOWOS_USER={self.query_one('#username_input', Input).value}/' disk_formatter.sh")

            await self.run_command(f"sed -i 's/export BOWOS_PASSWORD=6255/export BOWOS_PASSWORD={self.query_one('#password_input', Input).value}/' disk_formatter.sh")
